...
async function sendViaMailchimp(mailOptions) {
    ...
    if (!apiKey) {
        throw new Error('MAILCHIMP_API_KEY is required when EMAIL_PROVIDER=mailchimp');
    }

    if (typeof fetch !== 'function') {
        throw new Error('Global fetch is unavailable; Mailchimp provider requires Node 18+');
    }
    ...
    if (recipients.length === 0) {
        throw new Error('Mailchimp send requires at least one recipient');
    }
    ...
    try {
        ...
        if (!Array.isArray(result)) {
            throw new Error('Unexpected Mailchimp response format');
        }
