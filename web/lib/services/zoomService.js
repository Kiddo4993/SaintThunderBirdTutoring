function buildFallbackMeeting(durationMinutes = 60) {
    ...
    console.warn('⚠️ Zoom API credentials not configured. Session created without a meeting link.');
    return { provider: 'fallback', id: meetingId, password: '', joinUrl: '', durationMinutes };
}
...
async function createZoomMeeting({ topic, durationMinutes = 60, startTime }) {
    ...
    if (!token) return buildFallbackMeeting(durationMinutes);
...
async function createSessionMeeting({ topic, durationMinutes, startTime }) {
    try {
        ...
        console.error('⚠️ Zoom API unavailable, using fallback:', error.message);
